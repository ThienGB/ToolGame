...
class AutoClickerInstance:
    ...
    def input_text_robust(self, text):
        if not text: return
        # Tách chuỗi thành các lệnh riêng biệt để tránh shell hiểu lầm các ký tự đặc biệt như &, |, <, >
        commands = []
        ...
        if commands:
            # Gộp các lệnh bằng dấu chấm phẩy để chạy trong 1 phiên shell
    ...
    def run(self, accounts, modes, worker_index, shared_data):
        ...
        login_script = [
            ...
            {"action": "click_image", "target": "images/xong.jpg", "timeout": 30, "confidence": 0.7},
