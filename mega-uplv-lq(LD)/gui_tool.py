...
class AutoClickerInstance:
    ...
    def restart_emulator(self):
        ...
        # --- CHIẾN LƯỢC RESTART ---
        # 1. Thêm Jitter để tránh các tab cùng lúc nhảy vào lock quá nhiều
        jitter = random.uniform(5, 45)
        self.log(f"Đang chờ {jitter:.1f}s jitter trước khi xếp hàng restart...")
        time.sleep(jitter)

        # 2. Sử dụng Lock toàn cục để tránh việc gọi quit/launch ldconsole quá nhiều cùng lúc gây rớt lệnh
        ...
        with lock:
            ...
            time.sleep(2)
            self.log(f"Đang bật máy ảo (Index {index})...")
            try:
        ...
        while True:
            ...
            # Cập nhật trạng thái máy từ list2
            ...
            # Tự động kick launch nếu vm bị xịt tắt ngang sau khi đã gọi launch
            if not is_vm_running and elapsed > 15:
                self.log("Phát hiện máy ảo chưa chạy, gọi lệnh launch lại...")
                try: 
                    # Dùng chung lock để đảm bảo ldconsole không bị nghẽn
                    with lock:
                        subprocess.run([self.ld_console_path, "launch", "--index", str(index)], creationflags=subprocess.CREATE_NO_WINDOW, timeout=10)
                except: pass
                time.sleep(5)
            ...
            # Kiểm tra xem device_id đã online chưa
            try:
            ...
            if int(elapsed) % 30 == 0 and int(elapsed) > 0:
                self.log(f"Vẫn đang đợi máy ảo {index} (đã đợi {int(elapsed)}s)...")

            # Giới hạn an toàn 10 phút
            if elapsed > 600:
                self.log(f"!! LỖI: Không thể kết nối máy ảo {index} sau 10 phút.")
                return False
    ...
    def run(self, accounts, modes_func, worker_index, shared_data):
            ...
            
        {
                ...
                "cases": [
                    ...
                    {
                        "trigger1": "images/any2.png" ,
                        "confidence": 0.7,
                         "script": [
                            {"action": "click_image_if", "target": "images/any2.png", "timeout": 10, "confidence": 0.9},
                            {"action": "click_image", "target1": "images/vao.png","target2": "images/vao_button.png","target3": "images/vaoo.png", "timeout": 20, "confidence": 0.9},
                            {"action": "click_image_if", "target1": "images/vao.png","target2": "images/vao_button.png","target3": "images/vaoo.png", "timeout": 5, "confidence": 0.9},
                    ...
                    },
                    {
                        ...
                        {"action": "click_image", "target": "images/login_garena.png", "timeout": 420, "confidence": 0.9},
                        {"action": "click_image", "target": "images/vaohuongdan.png", "timeout": 420, "confidence": 0.9},
                        {"action": "click_image", "target1": "images/dkysau.png", "target2": "images/dang_ky_sau.png", "target3": "images/dang_ky_sau1.png", "target4": "images/skip.png", "confidence": 0.7},
        ...
        dinh_game_script = [
            ...
            {"action": "wait", "timeout": 2},
            {"action": "click_image_if", "target": "images/tuong5.png", "timeout": 15, "confidence": 0.9},
        ...
        shared_battle_script = [
            ...
            {"action": "wait", "timeout": 3},
            {"action": "click_image_if", "target1": "images/sansang5v5.png", "target2": "images/sansang3.png","timeout": 5, "confidence": 0.9},
        ...
        uplevel_script = [
            ...
            {"action": "click_image_if", "target": "images/cai_dat_button.png", "timeout": 5, "confidence": 0.9},
            {"action": "click_coords", "x": 833, "y": 22, "timeout": 1},
            {"action": "click_coords", "x": 833, "y": 22, "timeout": 1},
            {"action": "click_coords", "x": 833, "y": 22, "timeout": 1},
